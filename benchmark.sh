...
RX1_PID=$!
sleep 0.5
python3 sender.py --target-port 9205 --local-port 9305 --packets 120 --rto 0.20 --cc aimd --min-window 1 --max-window 32 > tx1.log 2>&1
kill $RX1_PID
...
RX2_PID=$!
sleep 0.5
python3 sender.py --target-port 9206 --local-port 9306 --packets 120 --window-size 8 --rto 0.20 --cc q-learning --q-epsilon 0.0 --q-table q_table.json > tx2.log 2>&1
kill $RX2_PID
