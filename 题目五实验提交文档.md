...
| `artifacts/models/active/q_table.json` | 当前使用的 Q-Learning Q-Table，可作为预训练模型权重提交 |
| `artifacts/models/active/dqn_model.pt` | 当前使用的 DQN 模型权重 |
| `artifacts/models/candidates/` | curriculum 训练产生、尚未安装为 active 的候选权重 |
| `artifacts/models/backups/` | 训练前或整理时备份的 Q-Table 与 DQN 权重 |
| `artifacts/metrics/metrics.csv` | 手动实验每轮的汇总指标 |
| `artifacts/metrics/history.csv` | 手动实验过程中 CWND 和 RTT 的时间序列记录 |
| `artifacts/plots/` | 手动绘图输出目录 |
| `artifacts/legacy/` | 旧版零散输出归档目录 |
...
flowchart LR
    ...
    Receiver --> Ack["累计 ACK"]
    Ack --> Sender
    Sender --> Metrics["artifacts/metrics/metrics.csv<br/>artifacts/metrics/history.csv"]
...
该奖励鼓励更高吞吐，同时惩罚高 RTT 和丢包。当前使用的训练结果保存在 `artifacts/models/active/q_table.json` 中，备份权重保存在 `artifacts/models/backups/`。由于本项目采用表格型 Q-Learning，`artifacts/models/active/q_table.json` 即为题目要求中的预训练 Q-Table/模型权重文件。
...
实验分别运行 AIMD 和 Q-Learning 两种拥塞控制模式，每轮发送 300 个 UDP 数据分组，使用虚拟瓶颈链路模拟网络拥塞。发送端记录每轮实验的吞吐量、平均 RTT、重传次数等指标，并将 CWND/RTT 时间序列写入 `artifacts/metrics/history.csv`。

AIMD 测试命令：

```powershell
python sender.py --target-host 127.0.0.1 --target-port 9001 --cc-mode aimd --window-size 1 --max-cwnd 80 --packets 300 --rto 0.2 --metrics-file artifacts/metrics/metrics.csv --history-file artifacts/metrics/history.csv
```

Q-Learning 测试命令：

```powershell
python sender.py --target-host 127.0.0.1 --target-port 9001 --cc-mode qlearning --window-size 1 --max-cwnd 80 --packets 300 --rto 0.2 --qtable-file artifacts/models/active/q_table.json --metrics-file artifacts/metrics/metrics.csv --history-file artifacts/metrics/history.csv
```

绘图命令：

```powershell
python plot_metrics.py --metrics-file artifacts/metrics/metrics.csv --history-file artifacts/metrics/history.csv --output artifacts/plots/comparison_required.png --smooth-window 5
```
...
- `artifacts/plots/comparison_required_cwnd.png`：CWND 随时间变化曲线。
- `artifacts/plots/comparison_required_throughput_rtt.png`：吞吐量与平均 RTT 柱状图。
- `artifacts/plots/comparison_required.png`：上述两类图的合并版本。

### 3.4 结果分析

从 CWND 曲线看，AIMD 在传输初期不断增大窗口，随后由于虚拟瓶颈链路队列压力上升出现丢包或重传，窗口被乘性减小，形成典型的锯齿波。该现象符合 AIMD 的拥塞避免特征。

Q-Learning 在读取预训练 Q-Table 后，将 CWND 控制在较低区间，但原始 CWND 曲线不一定完全光滑。这是因为训练和运行中存在 \(\epsilon\)-greedy 探索，题目要求的 6 个离散状态会压缩真实网络反馈，动作空间也只有保持、加一和减半三种离散选择；同时，RTT、丢包和 timeout 本身具有随机波动。因此，图中保留原始曲线，并叠加移动平均曲线观察整体趋势。测试中，Q-Learning 没有像 AIMD 一样快速推高窗口，因此减少了队列堆积和丢包风险；其吞吐量为 0.689756 Mbps，与 AIMD 的 0.695810 Mbps 接近，但平均 RTT 从 AIMD 的 117.103 ms 降低到 18.839 ms，并且重传次数为 0。
...
如果 `artifacts/models/active/q_table.json` 已存在，程序会读取该 Q-Table；如果不存在，程序会自动创建并保存。历史可用权重保存在 `artifacts/models/backups/`。
...
| 文件 | 内容 |
| --- | --- |
| `artifacts/plots/comparison_required.png` | 合并版图表 |
| `artifacts/plots/comparison_required_cwnd.png` | CWND 随时间变化曲线 |
| `artifacts/plots/comparison_required_throughput_rtt.png` | 吞吐量与平均 RTT 柱状图 |
